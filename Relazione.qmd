...
Per la classificazione binaria saranno analizzati i segnali temporali corrispondenti alla **mano a riposo** e **mano a pugno**, rimappati per il corretto funzionamento del modello rispettivamente come 0 ed 1. Inizialmente è stato valutato l'utilizzo dei segnali corrispondenti alla **mano aperti**, comportando però risultati inconcludenti dovuti alla scarsa presenza di dati riguardanti tale configurazione. Questo fenonemo provoca che il modello risponda continuamente "mano a pugno" avendo capito che la quantità di segnali riconducibili alla configurazione di "mano aperta" sono notevolemente inferiori alle configurazioni di "mano a pugno".

Approfittando della configurazione del dataset, si è optato per dividere gli individui usati per il *training* e quelli invece usati per il *testing* in modo da evitare in qualsiasi modo il ***Data Leakage.*** In particolare, i primi 24 soggetti saranno usati per allenare il modell, mentre i restanti 12 saranno usati per verificare l'allenamento. Questa configurazione permette di verificare il modello con pattern e segnali muscolari totalmente nuovi e mai visti durante la fase di training, costatandone l'accuratezza.

### 2.1 Segmentazione dei segnali $\implies$ Windowing

Per poter utilizzare i valori grezzi dei sensori al fine di allenare il modello di *Deep Learning,* è necessario innanzitutto caricare i risultati attraverso una tecnica di segmentazione del segnale. Questa consiste nell'uso di una "finestra temporale" che scorre nel dataframe, generando segmentazioni consecutive e parzialmente sovrapposte, in modo da garantire il rilevamento dell'evoluzione nel tempo del segnale, mantenendo comunque elevato numero di campioni per l'addestramento.
...
#### Segmento di codice dove si applica ciò che è stato definito anteriormente
...
Al fine di agevolare la convergenza dell'algoritmo durante la fase di addestramento del modello, è necessaria una normalizzazione dei valori grezzi ricavati dai sensori. I segnali da valutare infatti, variano a seconda di vari fattori soggettivi(forza dell'individuo, posizione leggermente diversa, ecc...) , per questo motivo non si è in grado di riconoscere un valore massimo e minimo nell'intensità del segnale( al contrario di quanto accade nel caso delle immagini, dove il colore dei pixel va da 0 a 255). Date queste premesse si è optato per la normalizzazione di tali valori, calcolando media e deviazione standard del dataset, esclusa la parte dedicata alla valutazione. Questa decisione nasce per evitare che il modello abbia informazioni riguardanti il dataset di *testing*. Media e deviazione standard $\mu$ vengono successivamente applicati ad entrambi i dataset, aggiungendo a $\mu$ una costante $\epsilon= 10^{-8}$ per evitare casi in cui la deviazione standard sia pari a 0.
...
Il 2° modello è una variazione del primo modello lenet1D, dove si elimina il terzo stadio convoluzionale. Questa variazione permette di ridurre i tempo dell'allenamento, grazie all'eliminazione di uno stadio. Questa velocità però, compromette l'accuratezza sia quando la differenza tra due movimenti risulta meno evidente, sia quando il numero di gesti da analizzare è maggiore.

Il modello, quindi, risulta più performante quando la classificazione rimane binaria, mentre l'accuratezza peggiora nel caso in cui sia necessario gestire ed analizzare molteplici gesti.

#### 3.2.1 Struttura

Come già accennato precedentemente, lo stile e il tipo di funzioni sono identici nei due modelli, con unica eccezione il numero di convoluzioni ridotto. La funzione **nn.Linear** avrà quindi un maggior numero di dati da analizzare, data la mancanza di un **nn.MaxPool1d**, il quale dimezzava la dimensione del tensore
...
Da questo punto in poi il modello non presenta ulteriori differenze rispetto agli altri presi in esame nei punti 3.1 e 3.2.
...
Come già definito precedentemente, per garantire che l'addestramento dell'algoritmo ne influenzi la valutazione, è stato deciso di separare i dati rispetto agli individui, cosicché la valutazione possa avvenire con segnali mai osservati e evitare l'*overfitting* per contaminazione dei dati. Seguendo questa logica sono stati separati anche i due programmi di *training* e *testing* a scopo organizzativo, migliorando l'interpretabilità di quest'ultimi.

### 4.0.1 Composizione training

Questo programma svolge il *training loop* su una serie di epoche. Il suo compito è alimentare l'algoritmo convoluzionale tramite i mini-batch, per poi calcolare la differenza tra le predizioni e la *ground truth*. Al fine di calcolare tale differenza, è stata scelta come *loss_function* la **Cross_Entropy_Loss**, poiché è la più adatta per i problemi di classificazione, aumentando il valore di *loss* in base alla gravità dell'errore di predizione. Successivamente, il valore di errore si propaga, aggiornando il valore dei pesi tramite l'ottimizzatore **Adam**, il più popolare tra gli ottimizzatori siccome regola il *learning rate* in base ai gradienti di ciascun peso, migliorando la convergenza della predizione.
...
Analizzando i temi di addestramento e l'accuratezza per ciascuna epoca, si può notare come la durata della prima epoca, nonché la sua accuratezza, siano drasticamente diverse rispetto alle restanti. Questo comportamento irregolare è dovuto principalmente ad operazioni di **allocazione della memoria(RAM/VRAM)**, dove Pytorch richiede l'assegnazione di blocchi di memoria contigui all'interno della VRAM della GPU nel quale saranno caricati i tensori dei batch e i pesi dell'ottimizzatore. Questa esecuzione non avviene per ogni epoca, ma solo durante la prima richiesta, dopo la quale vengono aggiornati i blocchi stessi.
...
Per valutare correttamente i risultati ottenuti è necessario innanzitutto definire i gesti analizzati: 0-mano a riposo, 1-mano a pugno, 2-flessione del polso, 3-estensione del polso, 4-deviazioni radiali, 5-deviazioni ulnari e 6-mano aperta. L'ultima configurazione non è stata svolta da tutti i partecipanti, perciò l'accuratezza calerà per quella determinata configurazione.

Siccome il numero di configurazioni da analizzare è maggiore rispetto alla classificazione binaria vista in precedenza, si utilizza la **matrice di confusione** per migliorare la lettura e la comprensione dell'accuratezza della predizione in base alla configurazione stessa.
...
-   per la configurazione 6 (mano aperta) non è stata predetta da nessuno dei 3 modelli. Questo **sbilanciamento del dataset** era già stato evidenziato dal creatore del dataset. Il modello quindi non è in grado di estrarre sufficienti *feature* e quindi classifica il movimento in maniera errata, confondendolo con configurazione similari(la deviazione ulnare).

-   Le configurazioni 2 e 5(flessione del polso e deviazione ulnirale), nonché 3 e 4 (estensione del polso e deviazione radiale), sono spesso confuse tra loro nel modello. Questo fenomeno è causato da **sinergia muscolare dei movimenti** e **bassa risoluzione degli strumenti di misura**. I muscoli che vengono attivati durante lo svolgimento di tali coppie sono molto simili tra loro, provocando una confusione nel modello di analisi. Per la precisione, la coppia 2-5 condivide l'attivazione del muscolo flessore ulnare del carpo, mentre la coppia 3-4 condivide l'attivazione degli estensori radiali del carpo. Siccome le fibre muscolari sono sovrapposte tra loro, i sensori notano segnali sovrapposti, che generano errate letture nel modello.

Per questa configurazione, il modello Lenet1D-2C, seppur mantenendo un tempo di allenamento rapido, è quello che presenta una minor accuratezza finale(ma questa differenza è facilmente trascurabile). Il miglior modello per accuratezza è il modello Multi-Scale1D, che permette di ottenere un'accuratezza massima di $78,40\%$.

## 6. Ottimizzazione del Modello e Risoluzione delle Criticità

A fronte delle problematiche emerse durante la classificazione multiclasse (7 gesti), nello specifico la mancata predizione della "mano aperta" e la confusione tra gesti sinergici, si è proceduto nell'introdurre tecniche per ridurre tali criticità e rendere l'architettura *Multi-Scale1D* più robusta.

### 6.1 Batch Normalization

Per aiutare il modello a distinguere i movimenti con elevata sinergia muscolare (es. flessione vs estensione del polso), è stata inserita la funzione `nn.BatchNorm1d` dopo ogni layer convoluzionale e prima dell'attivazione ReLU. Questa tecnica normalizza le attivazioni intermedie del tensore, stabilizzando i gradienti e permettendo ai layer di convoluzione successivi di concentrarsi sulle differenze microscopiche.

### 6.2 Dropout

Aumentando la difficoltà dei *task,* il rischio che il modello impari "a memoria" cresce notevolmente. Per ovviare a questo problema, prima dell'ultimo layer lineare di classificazione, è stato inserito un layer di **Dropout** con probabilità $p=0.2$. Disattivando casualmente la metà dei neuroni ad ogni iterazione, si forza l'algoritmo a non dipendere da un singolo sensore dominante, costringendolo a imparare una rappresentazione più distribuita e generalizzabile del gesto.

### 6.3 Learning Rate Scheduler

Siccome il tasso di apprendimento può portare l'ottimizzatore a "rimbalzare" attorno al minimo globale senza mai raggiungerlo. Si è quindi integrato `optim.lr_scheduler.ReduceLROnPlateau`. Questo strumento monitora l'errore di valutazione (*Validation Loss*): se la rete smette di migliorare per 8 epoche consecutive (*patience=8*), lo *scheduler* dimezza automaticamente il *learning rate* (`factor=0.5`). Ciò permette all'ottimizzatori di affinare sempre più il modello durante la fase di addestramento, differenziando meglio ancora le classi più muscolarmente sinergiche.

### 6.4 Weighted Random Sampler

Per risolvere la totale assenza di predizioni per la configurazione 6, causata dal forte sbilanciamento delle classi nel dataset originale, si è optato per una strategia di campionamento ponderato. Invece di alterare i dati con rumore artificiale, si è implementato il modulo **WeightedRandomSampler** di PyTorch. L'algoritmo calcola le occorrenze di ciascuna classe (*torch.bincount*) e assegna a ogni campione un peso inversamente proporzionale alla frequenza della propria classe d'appartenenza. Configurando il parametro *replacement=True*, il *DataLoader* provvede a un sovracampionamento (*oversampling*) dinamico durante l'estrazione: i mini-batch risultano così uniformemente distribuiti, costringendo il modello a calcolare gradienti robusti anche per la classe più critica.

### 6.5 Risultati

![](Images/Confusion_matrix_Multi-Scale1D_perf.png){width="90%"}

Valutando il modello sul Test Set, l'accuratezza globale è salita al **79.22%** (rispetto al precedente 78.40%), mantenendo un tempo di inferenza eccellente (0.25s).

Analizzando la nuova matrice di confusione, emergono due conclusioni fondamentali sullo stato del sistema, ossia il **limite fisiologico sui movimenti sinergici:** Si osserva un generale miglioramento della diagonale principale (predizioni corrette). Tuttavia, permane una fisiologica percentuale di errore tra le coppie di movimenti sinergici, in particolar modo tra la classe 4 (deviazione radiale) spesso confusa con la 3 (estensione), e tra la classe 5 (deviazione ulnare) confusa con la 2 (flessione). Da questo dato si conclude che l'algoritmo abbia raggiunto il limite fisico dell'hardware. La risoluzione spaziale fornita dai soli 8 sensori sEMG non contiene informazioni elettriche sufficienti per separare in modo netto l'attivazione di fibre muscolari fisicamente sovrapposte nell'avambraccio.

In conclusione, l'architettura *Multi-Scale1D* abbinata alle logiche di regolarizzazione (*Dropout*, *BatchNorm*) e bilanciamento (*WeightedSampler*) rappresentano un miglioramento generale dell'accuratezza senza rischio di *overfitting*, ma non è ancora in grado di riconoscere significativamente movimenti muscolarmente sinergici.
